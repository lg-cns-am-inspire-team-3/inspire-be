...
public class AuthController {
     ...
     */
    @PostMapping("/login")
    public ResponseEntity<String> login(HttpServletResponse servletResponse, @RequestBody UserLoginDTO userLoginDTO) {
        log.info("Login request received for ID: {}", userLoginDTO.getLoginId());
        
        authService.login(servletResponse, userLoginDTO);
        
        // 프론트엔드(Axios) 처리를 위해 리다이렉트 대신 성공 메시지 반환 [오후 9:22]]
    ...
    public ResponseEntity<TokenResponseDTO> reissue(
        ...
        TokenResponseDTO tokenResponse = authService.reissue(servletResponse, refreshToken);
